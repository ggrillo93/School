...
int main() {
    ...
    double K = 3.841e14, pi = 4.0 * atan(1.0), M = 1.9891e33, h, hold, start, stop, stepw, m = 0.0, T, centralT, mp = 1.672e-24, mu = 0.6, kb = 1.380e-16;
    bool cond = true;
    ...
    while (m < M) {
        ...
        if (h > hold || h == hold) { // means error is low enough, result is correct
            ...
            fp << m << setw(15) << initvar(0) << setw(15) << initvar(1) << setw(15) << T << endl;
            m = m + hold;
        }
    }
    fp.close();
    centralT = mu*mp*pow(presslist(minpos), 0.25)*pow(K, 0.75)/kb;
    cout << "Mass = " << m << "      " << "Radius = " << initvar(0) << "       " << "Central pressure = " << presslist(minpos) << "       " << "Central Temperature = " << centralT << endl;
    return (EXIT_SUCCESS);
}
