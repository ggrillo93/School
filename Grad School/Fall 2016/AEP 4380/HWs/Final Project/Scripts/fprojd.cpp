...
int main() {
	int N = 2, M = 300, i, j, k, n1 = 1, n2 = 1, nite = 0;
	double K = 2.478e14, totmass = 1.9891e33, G = 66732e-8, err = 0;
	...
	for (k = 0; k < M; k++) { y(M-1-k,1) = k*4e14/M; }
	...
	for (k = 0; k < M; k++) {
		err += abs(Evec(2*k-1)/(y(2*k-1)+0.01));
		err += abs(Evec(2*k)/(y(2*k)+0.01));
	}
	err /= Evec.n();
	cout << err << endl;
	while (err > 5000 && nite < 1000) {
...
double Ecomp0(arrayt<double>& y, arrayt<double>& mlist, double K) {
	...
	return y(0,0)-pow(K,1.0/5.0)*pow(mlist(0),1.0/3.0)/(pow(4*pi/3,1.0/3.0)*pow(y(0,1),1.0/5.0));
}

double EcompM(arrayt<double>& y, arrayt<double>& mlist, double K) {
	double G = 66732e-8, M = mlist(mlist.n()-1), R = y(mlist.n()-1, 0), pi = 4.0 * atan(1.0);
	return y(M-1,1)-y(0,1)+G*M*M/(8*pi*R*R*R*R);
}
...
void Sjn(int k, arrayt<double>& y, arrayt<double>& mlist, arrayt<double>& sjnmat, double K) {
	...
	if (k == 0) {
		sjnmat(0,0) = 1.0;
		sjnmat(0,1) = pow(3*mlist(0)/4*pi,1.0/3.0)*pow(K, 1.0/5.0)/(5*pow(y(0,1),6.0/5.0));
	}
	else if (k == M) {
		sjnmat(0,0) = -G*(mlist(M-1)*mlist(M-1))/(2*pi*pow(y(M-1,0),5.0));
		sjnmat(0,1) = 1.0;
	}
	else {
		sjnmat(0,0) = S00;
